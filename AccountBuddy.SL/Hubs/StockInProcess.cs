...
namespace AccountBuddy.SL.Hubs
{
    public partial class ABServerHub
    {
        ...
        public string StockInProcess_NewRefNo(DateTime dt)
        {
            string Prefix = string.Format("{0}{1:yy}{2:X}", BLL.FormPrefix.StockInProcess, dt, dt.Month);
            long No = 0;

            var d1 = DB.StockInProcesses.Where(x => x.Staff.Ledger.AccountGroup.CompanyId == Caller.CompanyId && x.RefNo.StartsWith(Prefix) && x.SPDate.Month == dt.Month).Select(x => x.RefNo).ToList();
            if (d1.Count() > 0)
            {
        ...
        public bool StockInProcess_Save(BLL.StockInProcess SO)
        {
            try
            {
                ...
                else
                {
                    ...
                    //}
                    decimal rd = SO.STPDetails.Select(X => X.SPId).FirstOrDefault();
