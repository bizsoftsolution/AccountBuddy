    ...
    public partial class ABServerHub
    {
		...
		public bool Sales_Save(BLL.Sale P)
        {
            try
            {
                ...
                Journal_SaveBySales(d, s);
                Purchase_SaveBySales(d, s);
                return true;
            }
            catch (Exception ex)
            {
        ...
        #region Purchase
        void Sales_SaveByPurchase(DAL.Purchase P, List<BLL.TaxMaster> TaxDetail)
        {
            try
            {
                ...
                if (P.Ledger.LedgerName.StartsWith("CM-") || P.Ledger.LedgerName.StartsWith("WH-") || P.Ledger.LedgerName.StartsWith("DL-"))
                {
                    ...
                    if (LId != 0)
                    {
                        ...
                        DB.SaveChanges();
                        Journal_SaveBySales(s, TaxDetail);
                    }
                }
            }
            catch (Exception ex)
        ...
        {
            BLL.Sale S = d.ToMap(new BLL.Sale());
            foreach (var d_Sd in d.SalesDetails)
            {
                S.SDetails.Add(d_Sd.ToMap(new BLL.SalesDetail()));
            }
            return S;
        }
