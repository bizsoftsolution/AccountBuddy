...
namespace AccountBuddy.SL.Hubs
{
    public partial class ABServerHub
    {
        ...
        public string JobOrderReceived_NewRefNo(DateTime dt)
        {
            string Prefix = string.Format("{0}{1:yy}{2:X}", BLL.FormPrefix.JobOrderReceived, dt, dt.Month);
            long No = 0;

            var d1 = DB.JobOrderReceiveds.Where(x => x.JobWorker.Ledger.AccountGroup.CompanyId == Caller.CompanyId && x.RefNo.StartsWith(Prefix) && x.JRDate.Month == dt.Month).Select(x => x.RefNo).ToList();
            if (d1.Count() > 0)
            {
