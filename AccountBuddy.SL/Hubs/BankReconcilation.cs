...
namespace AccountBuddy.SL.Hubs
{
    public partial class ABServerHub
    {
        ...
        {
            List<BLL.BankReconcilation> lstGeneralLedger = new List<BLL.BankReconcilation>();
            ...
            foreach (var l in lstLedger)
            {
                ...
                foreach (var pd in l.PaymentDetails.Where(x => x.Payment.PaymentDate >= dtFrom && x.Payment.PaymentDate <= dtTo && x.Payment.Status == "Proccess").ToList())
                {
                    gl = new BLL.BankReconcilation();
                    gl.Ledger = new BLL.Ledger();
                ...
                foreach (var p in l.Payments.Where(x => x.PaymentDate >= dtFrom && x.PaymentDate <= dtTo && x.Status == "Proccess").ToList())
                {
                    gl = new BLL.BankReconcilation();
                    gl.Ledger = new BLL.Ledger();
                ...
                foreach (var r in l.Receipts.Where(x => x.ReceiptDate >= dtFrom && x.ReceiptDate <= dtTo && x.Status == "Process").ToList())
                {
                    gl = new BLL.BankReconcilation();
                    gl.Ledger = new BLL.Ledger();
                ...
                foreach (var rd in l.ReceiptDetails.Where(x => x.Receipt.ReceiptDate >= dtFrom && x.Receipt.ReceiptDate <= dtTo && x.Receipt.Status=="Process").ToList())
                {
                    gl = new BLL.BankReconcilation();
                    gl.Ledger = new BLL.Ledger();
