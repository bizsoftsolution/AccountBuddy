...
namespace AccountBuddy.SL.Hubs
{
    public partial class ABServerHub
    {
        ...
        public string Receipt_NewRefNo(DateTime dt)
        {
            ...
            var d1 = DB.Receipts.Where(x => x.Ledger.AccountGroup.CompanyId == Caller.CompanyId && x.EntryNo.StartsWith(Prefix) && x.ReceiptDate.Month == dt.Month).Select(x => x.EntryNo).ToList();
            if (d1.Count() > 0)
            {
